...
test_that("ChanFacebook Network Analysis", {
  # Define the path to the dataset
  dataset_path <- system.file("extdata", "ChanFacebook.gml", package = "arlclustering")
  if (dataset_path == "") {
    stop("ChanFacebook.gml file not found in the package")
  }
  ...
  timings[["Load Network"]] <- microbenchmark(
    g <- arlc_get_network_dataset(dataset_path, "Chan Facebook"),
    times = 1
  )
  ...
  ggplot(timing_data, aes(x = Step, y = Time)) +
    ...
    labs(title = "Execution Time of Steps in Chan Facebook Network Analysis",
         x = "Step",
         y = "Time (ms)") +
