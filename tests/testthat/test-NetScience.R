...
test_that("NetScience Network Analysis", {
  # Define the path to the dataset
  dataset_path <- system.file("extdata", "netscience.gml", package = "arlclustering")
  if (dataset_path == "") {
    stop("netscience.gml file not found in the package")
  }
  ...
  if (!is.finite(maxLenRules) || maxLenRules > 5*length(transactions)) {
    paste0("Invalid maxLenRules: must be finite and > minLenRules ")
  ...
  ggplot(timing_data, aes(x = Step, y = Time)) +
    ...
    labs(title = "Execution Time of Steps in NetScience Network Analysis",
         x = "Step",
         y = "Time (ms)") +
